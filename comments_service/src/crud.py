...
async def comments_by_user(user_id: int, collection: AsyncIOMotorCollection,
                           parameters: tuple[int, int, list[tuple[str, int]] | None, dict[str, Any] | None]) -> \
        list[Comment]:
    offset, limit, order_by, where = parameters
...
async def count_comments_by_user(user_id: int, collection: AsyncIOMotorCollection,
                                 parameters: tuple[
                                     int, int, list[tuple[str, int]] | None, dict[str, Any] | None]) -> int:
    if (result := await comments_by_user(user_id=user_id, collection=collection, parameters=parameters)) is not None:
...
async def count_comments_by_post(post_id: str, collection: AsyncIOMotorCollection,
                                 parameters: tuple[
                                     int, int, list[tuple[str, int]] | None, dict[str, Any] | None]) -> int:
    if (comments := await comments_by_post(post_id=post_id, collection=collection, parameters=parameters)) is not None:
