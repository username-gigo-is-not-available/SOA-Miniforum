...
from src.pubsub.config import USER_CREATED_TOPIC, USER_DELETED_TOPIC
...
async def create(user: UserCreate, cache: Redis) -> User:
    try:
        ...
        if e.status_code == 404:
            ...
            logger.info(
                f"Sending key: {result.id} value: {result.dict()} to topic: {USER_CREATED_TOPIC}")
            await get_producer().send(topic=USER_CREATED_TOPIC, value=json.dumps(result.dict()).encode('utf-8'))
...
async def delete(user_id: int, cache: Redis) -> User:
    ...
    if user and await cache.hdel(get_user_hash_name(user_id=user_id), *user.keys()) > 0:
        ...
        logger.info(
            f"Sending key: {user['id']} value: {user} to topic: {USER_DELETED_TOPIC}")
        await get_producer().send(topic=USER_DELETED_TOPIC, value=json.dumps(user).encode('utf-8'))
        return User(**user)
    else:
        logger.exception(user_not_found_message(user_id=user_id))
        raise HTTPException(status_code=404, detail=user_not_found_message(user_id=user_id))
...
async def query(cache: Redis, email_pattern: str = r'.*') -> list[User]:
    result = []
